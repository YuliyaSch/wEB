...
function numbButton(x){
	if (blackNumb === "Infinity") {alert('к бесконечности нельзя приписать число !!!!!!!')}
		...
		if (x == 10) {x = ".";}
		if (blackNumb == "" && x == '.') {alert("нельзя начинать число с точки");}
		else if (pointerCheck == '.' && x == '.') {alert("нельзя иметь больше одной точки в числе");}
		else{
			sdvig +=16;
...
function operButton(x){
	...
	else if (blackNumb === '-') {alert('напишите циферки');}
...
function equationButton(){
	if (blackNumb == "" || blackNumb.slice(-1) === '.') {alert("закончите выражение пожалуйста !!!!!")}
		...
		prov2=0;
		if (operator === '+'){blackNumb = Number(number1) + Number(blackNumb);}
		else if (operator === '-'){blackNumb = Number(number1) - Number(blackNumb);}
		else if (operator === '*'){blackNumb = Number(number1) * Number(blackNumb);}
		else if (operator === '/'){blackNumb = Number(number1) / Number(blackNumb);}
